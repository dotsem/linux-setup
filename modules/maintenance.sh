...
setup_maintenance() {
    ...
    if sudo systemctl enable --now fstrim.timer 2>>"$LOG_FILE"; then
    ...
    case "$DETECTED_PKG_MANAGER" in
        pacman)
            if sudo pacman -S --noconfirm pacman-contrib 2>>"$LOG_FILE"; then
            ...
            ;;
        dnf)
            sudo dnf install -y dnf-automatic 2>>"$LOG_FILE"
            sudo systemctl enable --now dnf-automatic.timer 2>>"$LOG_FILE"
            log "INFO" "Configured DNF automatic updates"
            print_status success "DNF automatic updates enabled"
            ;;
        apt)
            sudo apt-get install -y unattended-upgrades 2>>"$LOG_FILE"
            sudo dpkg-reconfigure -plow unattended-upgrades 2>>"$LOG_FILE" || true
            log "INFO" "Configured unattended upgrades"
            print_status success "Unattended upgrades enabled"
            ;;
    esac

    sudo sed -i 's/^#SystemMaxUse=/SystemMaxUse=100M/' /etc/systemd/journald.conf 2>>"$LOG_FILE"
